...
scikit-learn公式ドキュメントでは、ハイパーパラメータは「estimatorの内部で直接学習されず、estimatorクラスのconstructor引数として渡されるもの」と説明されている。

この章の各モデル表で扱う `C`, `max_depth`, `n_estimators`, `learning_rate` などは、基本的に **ハイパーパラメータ**。

### 質問への回答

#### 1. パラメータは人間が絶対にいじらない領域か

基本的には、**直接いじらない** と考えてよい。

たとえば、ロジスティック回帰の係数や、決定木の具体的な分岐条件は、学習データからモデルが決める。
...
ただし、「絶対に触れない」というより、通常の機械学習ワークフローでは直接編集しない、という理解が正確。

人間が直接係数を書き換えるのではなく、次のものを調整して、学習結果としてのパラメータを変える。
...
#### 2. モデルが決めたパラメータに人間の意思を入れない制約はあるか

厳密な意味で「人間の意思を一切入れない」という制約はない。
...
ただし、学習済みのパラメータを人間が手で都合よく編集するのは、通常は避ける。

理由:

- 再現性が下がる
- 評価結果の意味が曖昧になる
- 学習アルゴリズムで最適化した結果ではなくなる
- 後からなぜその値になったのか説明しにくい

人間が意思を入れるべき場所は、学習済みパラメータそのものではなく、設計と検証の段階。
...
避けたい介入:
  学習後の係数を手で書き換えて、都合のよい予測にする
```

#### 3. 表にあるものはパラメータかハイパーパラメータか

この章で「主な調整ハイパーパラメータ」として表に載せているものは、ほぼすべて **ハイパーパラメータ**。

つまり、人間がモデル作成前に設定・調整するもの。

一方で、モデルが学習後に持つパラメータは、scikit-learnでは名前の末尾に `_` が付くことが多い。

例:

| モデル | 学習後のパラメータ例 | 人間が通常直接調整するか |
|---|---|---|
| ロジスティック回帰 | `coef_`, `intercept_` | 直接は調整しない |
| 決定木 | `tree_`, `feature_importances_` | 直接は調整しない |
| Random Forest | `estimators_`, `feature_importances_` | 直接は調整しない |
| k近傍法 | `classes_`, `effective_metric_` | 直接は調整しない |

見分け方:

| 見分ける観点 | パラメータ | ハイパーパラメータ |
|---|---|---|
| いつ決まるか | 学習後 | 学習前 |
| 誰が決めるか | モデル | 人間 |
| scikit-learnでの位置 | 学習後の属性に多い | constructor引数に多い |
| 名前の例 | `coef_`, `tree_` | `C`, `max_depth` |
| 調整するか | 通常は直接調整しない | 調整対象になる |

このため、以下の章では「主な調整ハイパーパラメータ」と表記する。

### 調整の基本方針

最初から大量のハイパーパラメータを調整しない。
